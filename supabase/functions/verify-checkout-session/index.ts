...
async function resolveCommissionPct(
  ...
  if (paidSessionCount < 10) {
    console.log(`[verify-checkout] Creator ${creatorId} free plan session ${paidSessionCount + 1}/10 → 0% commission`);
    return 0;
  }
  console.log(`[verify-checkout] Creator ${creatorId} free plan session ${paidSessionCount + 1} → 8% commission`);
  return 8;
}
