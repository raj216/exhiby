-- Tighten realtime topic access policy
...
TO authenticated
USING (
  -- User-scoped personal channels (exact match on auth.uid())
  ...
  -- Shared discovery signal channels (no user-identifiable data, just refetch triggers)
