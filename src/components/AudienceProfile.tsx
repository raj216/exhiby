...
export function AudienceProfile({
  ...
  const {
    stats
  } = useAudienceStats(user?.id);
  const {
    stats: followStats
  } = useFollowStats(user?.id);
  const {
    upcomingSessions: ticketSessions,
    pastSessions,
  ...
  return <div className="min-h-screen bg-carbon">
      ...
      <div className="max-w-screen-xl mx-auto lg:px-8">
        ...
        <div className="relative h-48 sm:h-56 w-full overflow-hidden">
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-10">
            ...
            {isVerifiedCreator && onSwitchMode && <motion.button initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} onClick={e => {
        ...
        <div className="border-b border-border/30 mt-6">
          <div className="flex max-w-2xl mx-auto lg:justify-center lg:gap-4 px-4">
            ...
            setActiveTab(tab.id);
                ...
                {tab.label}
                {activeTab === tab.id && <motion.div layoutId="audienceTab" className="absolute bottom-0 left-2 right-2 h-0.5 bg-destructive" />}
              </button>)}
          </div>
        </div>
        ...
        <div className="pb-24 lg:pb-8 max-w-4xl mx-auto">
          <AnimatePresence mode="wait">
            {/* Tickets Tab */}
            {activeTab === "tickets" && <motion.div key="tickets" initial={{
            opacity: 0,
            y: 10
          }} animate={{
            opacity: 1,
            ...
            y: -10
                ...
                {(() => {
              const upcomingSaved = savedSessions.filter(s => s.status === "upcoming" || s.status === "starting_soon" || s.status === "live");
              const pastSaved = savedSessions.filter(s => s.status === "ended" || s.status === "missed");
              const upcomingTickets = ticketSessions.filter(s => s.status === "upcoming" || s.status === "starting_soon" || s.status === "live");
              const pastTickets = ticketSessions.filter(s => s.status === "ended" || s.status === "missed");
              ...
              const renderStatusBadge = (status: string, attended?: boolean) => {
                switch (status) {
                  case "live":
                    return <span className="px-2 py-0.5 rounded-full bg-destructive text-destructive-foreground text-xs font-medium flex items-center gap-1">
                            <Radio className="w-3 h-3" />
                            LIVE
                          </span>;
                  case "starting_soon":
                    return <span className="px-2 py-0.5 rounded-full bg-electric/15 text-electric text-xs font-medium flex items-center gap-1">
                  ...
                  case "upcoming":
                    ...
                    return null;
                  case "ended":
                    return <span className="px-2 py-0.5 rounded-full bg-muted/50 text-muted-foreground text-xs font-medium flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3" />
                            Attended
                          </span>;
                  case "missed":
                    return <span className="px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-xs font-medium flex items-center gap-1">
              ...
              const toggleSaved = async (params: {
                ...
                const ok = alreadySaved ? await removeSession(eventId) : await saveSession(eventId, creatorId);
                if (ok) {
                  toast({
                    title: alreadySaved ? "Removed" : "Added",
                    description: alreadySaved ? "Removed from your Upcoming" : "Added to your Upcoming"
                  });
              ...
              return <>
                      ...
                      {ticketsLoading || savedLoading ? <div className="flex flex-col items-center justify-center py-12 bg-obsidian rounded-2xl border border-border/30">
                          <div className="w-8 h-8 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />
                        </div> : upcomingSaved.length > 0 || upcomingTickets.length > 0 ? <div className="space-y-3">
                          {/* Saved Sessions (upcoming only) */}
                          {upcomingSaved.map(session => <motion.div key={`saved-${session.id}`} initial={{
                    opacity: 0,
                    y: 10
                  }} animate={{
                    opacity: 1,
                    y: 0
                  }} onClick={() => handleSessionClick(session, true)} className={`bg-obsidian rounded-xl border p-4 cursor-pointer transition-colors ${session.status === "live" ? "border-destructive/50 bg-destructive/5" : "border-border/30 hover:border-border/50"}`}>
                              <div className="flex items-start gap-3">
                                <img src={getEventThumbnailUrl({
                        eventCoverUrl: session.coverUrl,
                        creatorAvatarUrl: session.artistAvatar
                      })} alt={session.title} className="w-10 h-10 rounded-full object-cover" />
                              ...
                              minute: "2-digit"
                                ...
                                {session.status === "upcoming" ? <button onClick={e => toggleSaved({
                        e,
                        eventId: session.eventId,
                        creatorId: session.creatorId
                      })} className="px-2 py-0.5 rounded-full bg-muted/50 text-muted-foreground text-xs font-medium">
                                    Added ✓
                                  </button> : renderStatusBadge(session.status)}
                              </div>
                            </motion.div>)}
                          
                          {/* Ticket Sessions (upcoming only) */}
                          {upcomingTickets.map(session => <motion.div key={`ticket-${session.id}`} initial={{
                    opacity: 0,
                    y: 10
                  }} animate={{
                    opacity: 1,
                    y: 0
                  }} onClick={() => handleSessionClick(session, false)} className={`bg-obsidian rounded-xl border p-4 cursor-pointer transition-colors ${session.status === "live" ? "border-destructive/50 bg-destructive/5" : "border-border/30 hover:border-border/50"}`}>
                              <div className="flex items-start gap-3">
                                <img src={getEventThumbnailUrl({
                        eventCoverUrl: session.coverUrl,
                        creatorAvatarUrl: session.artistAvatar
                        ...
                        </div> : <div className="flex flex-col items-center justify-center py-12 bg-obsidian rounded-2xl border border-border/30">
                      ...
                      {ticketsLoading || savedLoading ? <div className="flex flex-col items-center justify-center py-8 bg-obsidian/50 rounded-2xl border border-border/20">
                          <div className="w-6 h-6 border-2 border-muted-foreground/30 border-t-foreground rounded-full animate-spin" />
                        </div> : pastSaved.length > 0 || pastSessions.length > 0 || pastTickets.length > 0 ? <div className="space-y-2">
                          {/* Missed/Ended saved sessions with remove button */}
                          {pastSaved.map(session => <motion.div key={`past-saved-${session.id}`} initial={{
                    opacity: 0,
                    y: 10
                  }} animate={{
                    opacity: 1,
                    ...
                    opacity: 0,
                    x: -20
                  }} onClick={() => handleSessionClick(session, true)} className={`bg-obsidian/50 rounded-xl border p-4 cursor-pointer transition-colors ${session.status === "missed" ? "border-muted-foreground/30 opacity-70" : "border-border/20 hover:border-border/40"}`}>
                              <div className="flex items-start gap-3">
                                {session.artistAvatar ? <img src={session.artistAvatar} alt={session.artistName} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-muted-foreground">
                              ...
                              day: "numeric"
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button onClick={e => handleRemoveSession(e, session.eventId)} className="p-1.5 rounded-full hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" title="Remove from list">
                          ...
                          {pastSessions.map(session => <div key={`past-ticket-${session.id}`} className="bg-obsidian/50 rounded-xl border border-border/20 p-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-sm font-medium text-foreground">{session.artistName}</p>
                                  <p className="text-xs text-muted-foreground">
                                    {session.category || "Studio Session"} · {session.attendedAt.toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                            year: "numeric"
                          })}
                        ...
                        </div> : <div className="flex flex-col items-center justify-center py-8 bg-obsidian/50 rounded-2xl border border-border/20">
      ...
      <AnimatePresence>
        {selectedEndedSession && <SessionEndedScreen title={selectedEndedSession.title} artistName={selectedEndedSession.artistName} artistAvatar={selectedEndedSession.artistAvatar} scheduledAt={selectedEndedSession.scheduledAt} creatorId={selectedEndedSession.creatorId} isMissed={selectedEndedSession.status === "missed"} onRemove={async () => {
