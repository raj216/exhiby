...
export function ShareStudioModal({ isOpen, onClose, handle, userId, creatorName }: ShareStudioModalProps) {
  ...
  const getShareLinks = () => {
    ...
    return {
      ...
      instagram: `instagram://story-camera`, // Opens Instagram Stories camera - user copies link first
      tiktok: `https://www.tiktok.com/`, // TikTok doesn't have direct share URL - user copies link first
  ...
  const handleSharePlatform = async (platform: string) => {
    ...
    if ((platform === "instagram" || platform === "tiktok") && studioUrl) {
      try {
        await navigator.clipboard.writeText(studioUrl);
        toast({
            ...
            >
              ...
              <div className="px-5 pb-6">
                ...
                <motion.button
                  ...
                  className="w-full py-3.5 rounded-xl btn-electric flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {copied ? (
                    <>
                ...
                <div className="flex gap-3 mt-4">
                  <motion.button
                    ...
                    disabled={urlError || !studioUrl}
                    className="flex-1 py-3 rounded-xl bg-muted/50 border border-border/40 flex items-center justify-center gap-2 hover:bg-muted/70 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <motion.button
                    ...
                    disabled={urlError || !studioUrl}
                    className={`flex-1 py-3 rounded-xl border flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                      showQR
