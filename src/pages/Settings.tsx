...
const menuItems: MenuItem[] = [
  ...
  { id: "privacy", label: "Privacy & Safety", icon: Shield, group: "general" },
  // Group 2: The Studio (Creator Tools)
  { id: "payments", label: "Payments & Payouts", icon: CreditCard, group: "studio" },
  { id: "vacation", label: "Vacation Mode", icon: Palmtree, group: "studio" },
  { id: "shipping", label: "Shipping Addresses", icon: MapPin, group: "studio" },
  // Group 3: Support
  { id: "help", label: "Help Center", icon: HelpCircle, group: "support" },
    ...
    return (
      <div className="min-h-screen bg-carbon">
        ...
        <header className="sticky top-0 z-40 flex items-center gap-3 px-4 py-4 bg-carbon/95 backdrop-blur-xl border-b border-border/20">
          <button 
            onClick={handleBack}
            className="w-10 h-10 rounded-full bg-obsidian flex items-center justify-center"
          >
        ...
        <AnimatePresence mode="wait">
          {!activeCategory ? (
            ...
            >
              ...
              {/* The Studio Section */}
              {isVerifiedCreator && (
                <MenuSection title="The Studio" items={studioItems} onSelect={handleCategoryClick} />
              )}
  ...
  return (
    <div className="min-h-screen bg-carbon">
      ...
      <header className="sticky top-0 z-40 flex items-center gap-3 px-6 py-4 bg-carbon/95 backdrop-blur-xl border-b border-border/20">
        <button 
          onClick={handleBack}
          className="w-10 h-10 rounded-full bg-obsidian flex items-center justify-center hover:bg-muted/50 transition-colors"
        >
          <ArrowLeft className="w-5 h-5 text-foreground" />
        </button>
      ...
      <div className="space-y-1">
        {items.map((item) => (
          <button
            ...
            className="w-full flex items-center justify-between p-4 bg-obsidian rounded-xl border border-border/20 hover:border-border/40 transition-colors"
          >
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive 
                  ? "bg-electric/10 border-l-2 border-electric text-foreground" 
  ...
  return (
    <div className="space-y-4">
      <SettingsCard 
        ...
        onClick={() => toast({ title: "Email", description: "Email settings coming soon" })}
      />
      <SettingsCard 
        title="Password"
        description="Update your password"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Password", description: "Password settings coming soon" })}
      />
      <SettingsCard 
        title="Two-Factor Authentication"
        description="Add an extra layer of security"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "2FA", description: "2FA settings coming soon" })}
      />
      ...
      <div>
        ...
        <div className="space-y-3">
          <ToggleCard
            ...
            onToggle={() => handleToggle("email_live")}
            disabled={saving}
          />
          <ToggleCard
            ...
            onToggle={() => handleToggle("email_scheduled")}
            disabled={saving}
          />
          <ToggleCard
            title="Reminders"
            description="30 minutes before sessions you've saved"
            checked={preferences.email_reminders}
            onToggle={() => handleToggle("email_reminders")}
            disabled={saving}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          <ToggleCard
            ...
            onToggle={() => handleToggle("inapp_live")}
            disabled={saving}
          />
          <ToggleCard
            title="Scheduled sessions"
            ...
            onToggle={() => handleToggle("inapp_scheduled")}
            disabled={saving}
          />
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <SettingsCard 
        ...
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Blocked", description: "Blocked users coming soon" })}
      />
      <SettingsCard 
        title="Terms of Service"
        description="Read our terms and conditions"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Terms", description: "Opening terms..." })}
      />
      <SettingsCard 
        title="Privacy Policy"
        description="Read our privacy policy"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Privacy", description: "Opening privacy policy..." })}
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <SettingsCard 
        ...
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Payments", description: "Payment methods coming soon" })}
      />
      <SettingsCard 
        title="Payout Settings"
        description="Set up how you receive your earnings"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Payouts", description: "Payout settings coming soon" })}
      />
      <SettingsCard 
        title="Transaction History"
        description="View your payment and payout history"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "History", description: "Transaction history coming soon" })}
      />
    </div>
  );
}
  ...
  const handleToggle = () => {
    ...
    toast({ 
      ...
      description: vacationMode 
        ? "Your studio is now active" 
        : "Your studio is now on vacation"
    });
  };
  ...
  return (
    <div className="space-y-4">
      <SettingsCard 
        ...
        description="Add a shipping address for physical art"
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Address", description: "Address form coming soon" })}
      />
...
function HelpContent() {
  return (
    <div className="space-y-4">
      ...
      />
      <a 
        href="mailto:support@exhiby.com"
        className="block"
      >
        <SettingsCard 
      ...
      <SettingsCard 
        ...
        action={<ChevronRight className="w-5 h-5 text-muted-foreground" />}
        onClick={() => toast({ title: "Bug Report", description: "Bug report form coming soon" })}
      />
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div className="p-6 bg-carbon rounded-xl border border-border/20">
        ...
        <div className="space-y-4 text-sm text-muted-foreground">
          ...
          <button 
            onClick={() => toast({ title: "Guidelines", description: "Opening full guidelines..." })}
...
function SettingsCard({ 
  ...
  return (
    <button 
      onClick={onClick}
      className="w-full flex items-center justify-between p-4 bg-carbon rounded-xl border border-border/20 hover:border-border/40 transition-colors text-left"
    >
      <div>
...
function ToggleCard({ 
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-carbon rounded-xl border border-border/20">
      <div>
        <span className="text-foreground block">{title}</span>
        <span className="text-sm text-muted-foreground">{description}</span>
      </div>
